...
function addVideo() {
  let storageRef = firebase.storage().ref("VideoThumbail/" + fileName);
  ...
  uploadTask.on(
    ...
    () => {
      uploadTask.snapshot.ref.getDownloadURL().then((url) => {
        axios
          .post("https://sports-website-backend-production.up.railway.app/api/v4/add", {
            title: document.getElementById("video-title").value,
            paragraph: document.getElementById("video-para").value,
...
axios
  .get("https://sports-website-backend-production.up.railway.app/api/v4/admin/videos")
  .then((result) => {
    var i = 0;
    result.data.Videos.map((data) => {
...
function deleteVideo(id) {
  axios
    .delete(`https://sports-website-backend-production.up.railway.app/api/v4/admin/Video/${id}`)
    .then((result) => {
      console.log(result);
    })
    .catch((err) => {
